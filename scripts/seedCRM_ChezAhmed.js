...
const argId = process.argv
	.find((a) => a.startsWith("--restaurantId="))
	?.split("=")[1];
const RESTAURANT_ID = new mongoose.Types.ObjectId(
	argId || "686af511bb4cba684ff3b72e",
);
...
function randDateInPastDays(maxDaysAgo = 30) {
	const msAgo =
		randInt(0, maxDaysAgo) * 24 * 60 * 60 * 1000 +
	...
	for (const server of serverDocs) {
		...
		for (let i = 0; i < server.ordersCount; i++) {
			...
			const completedAt = new Date(
				createdAt.getTime() + serviceMinutes * 60 * 1000,
			);
			...
			const targetTotal = Math.round(
				randAround(server.avgTicket, 0.3) * nbPersonnes,
			);
		...
		console.log(
			`    ✅ ${server.ordersCount} commande(s) créée(s) pour ${server.name}`,
		);
	}
