/**
 * Script : Fermeture propre des résas "confirmed" de Chez Ahmed
 ...
 * 3. Passe le status de la résa à "completed"
 *
...
async function main() {
	...
	// 1. Charger toutes les résas "confirmed" + "pending" du restaurant
	...
	console.log(
		`📋 ${reservations.length} résa(s) "confirmed" + "pending" trouvée(s)\n`,
	);
	...
	// 4. Vérification finale : s'assurer qu'il ne reste plus aucune résa "confirmed"
	...
	if (remaining === 0) {
		console.log(
			'🎉 Vérification OK — plus aucune résa "confirmed" pour Chez Ahmed',
		);
	} else {
		console.warn(
			`⚠️  Attention : ${remaining} résa(s) encore "confirmed" — vérifier manuellement`,
		);
	}
