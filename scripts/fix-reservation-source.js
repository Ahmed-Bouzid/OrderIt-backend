 ...
 *           pour avoir reservationSource = "on_site"
 * 
...
async function fixReservationSource() {
	try {
		...
		for (const resa of reservationsWithoutSource) {
			resa.reservationSource = "on_site";
		...
		// Optionnel : Corriger les réservations qui ont "online" mais pas de date/heure planifiée
		...
		if (webReservationsWithoutDateTime.length > 0) {
			console.log(`\n⚠️  Trouvé ${webReservationsWithoutDateTime.length} réservations "online" sans date/heure planifiée`);
			console.log('Ces réservations devraient probablement être "on_site" :\n');
