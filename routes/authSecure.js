...
const { generateTokenPair, refreshTokens, revokeToken, revokeAllUserTokens } = require('../utils/jwtSecure');
const { asyncHandler, createError } = require('../middlewares/secureErrorHandler');
 ...
 */
router.post('/logout', authSecure, asyncHandler(async (req, res) => {
	const { refreshToken } = req.body;
 ...
 */
router.post('/logout-all', authSecure, asyncHandler(async (req, res) => {
